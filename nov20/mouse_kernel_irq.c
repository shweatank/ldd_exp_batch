...
#define DEVICE_NAME "interrupt_calc_dev_mouse"
...
printk(KERN_INFO "loading custom keyboard IRQ Handler ..\n");
     ...
     result = request_irq(MOUSE_IRQ, mouse_irq_handler, IRQF_SHARED,
                                       "mouse_irq_handler", (void *)(mouse_irq_handler));
        if(result)
           {
...
MODULE_DESCRIPTION("mouse IRQ Handler for x86 linux");
