...
#include <linux/clk.h>   //Clock framework for enabling, disabling, and managing hardware clocks//
#include <linux/delay.h> //delay functions like msleep() and udelay()//
#include <linux/device.h> //Core device model used by all kernel drivers//
#include <linux/dma-mapping.h> //DMA memory allocation and mapping for hardware access//
#include <linux/err.h>  //Standard macros for error handling and checking//
#include <linux/gpio/consumer.h> //GPIO descriptor API for safe GPIO access from drivers//
#include <linux/init.h>  //Initialization macros used during driver load time//
#include <linux/interrupt.h> //APIs for registering and handling hardware interrupts//
#include <linux/io.h> //Functions for memory-mapped I/O register access//
#include <linux/module.h> //Support for building the driver as a loadable kernel module//
#include <linux/of_device.h> //Device Tree matching and platform data access//
#include <linux/of_graph.h> //Parses Device Tree media graph connections//
#include <linux/pinctrl/consumer.h> //Pin multiplexing and pin configuration control//
#include <linux/platform_device.h> //Platform device registration and management//
#include <linux/pm_runtime.h> //Runtime power management for power-efficient devices//
#include <linux/slab.h> //Dynamic memory allocation using kmalloc and kzalloc//
#include <linux/uaccess.h> //Safe data transfer between kernel and user space//
#include <linux/videodev2.h> //Core V4L2 definitions for video devices//

#include <media/mipi-csi2.h> //Definitions for MIPI CSI-2 camera interface//
#include <media/v4l2-common.h> //Common helper functions used across V4L2 drivers//
#include <media/v4l2-ctrls.h> //Camera controls such as brightness, contrast, and exposure//
#include <media/v4l2-dev.h>  //V4L2 device node creation and management//
#include <media/v4l2-device.h> //V4L2 device registration with the kernel//
#include <media/v4l2-dv-timings.h> //Digital video timing and resolution handling//
#include <media/v4l2-event.h> //Event notification support for userspace//
#include <media/v4l2-ioctl.h> //IOCTL command handling for V4L2 operations//
#include <media/v4l2-fwnode.h> //Firmware and Device Tree based media endpoint parsing//
#include <media/videobuf2-dma-contig.h> //DMA-contiguous video buffer management//

#include <media/v4l2-async.h> //Asynchronous registration of camera sub-devices//

#include "vc4-regs-unicam.h" //Hardware register definitions specific to the Unicam controller//
...
//Defines a constant table of supported pixel formats used by the Unicam driver//
...
V4L2	 * if we have no other queued buffers to swap to.
	 */
...
/* ISR handler for Unicam capture, triggered on hardware interrupt */
static irqreturn_t unicam_isr(int irq, void *dev)
{
    struct unicam_device *unicam = dev;       // Get Unicam device structure from dev pointer
    unsigned int lines_done = unicam_get_lines_done(dev); // Number of lines captured so far
    unsigned int sequence = unicam->sequence; // Current frame sequence number
    unsigned int i;                            // Loop/index variable
    u32 ista, sta;                             // Interrupt status registers
    bool fe;                                   // Field end flag
    u64 ts;                                    // Timestamp for the frame

    sta = reg_read(unicam, UNICAM_STA);       // Read Unicam status register
    reg_write(unicam, UNICAM_STA, sta);       // Clear the status by writing back

    ista = reg_read(unicam, UNICAM_ISTA);     // Read interrupt status register
    reg_write(unicam, UNICAM_ISTA, ista);     // Clear the interrupt flags

    unicam_dbg(3, unicam, "ISR: ISTA: 0x%X, STA: 0x%X, sequence %d, lines done %d",
               ista, sta, sequence, lines_done); // Debug print ISR status

    if (!(sta & (UNICAM_IS | UNICAM_PI0)))   // If no relevant interrupt, exit ISR
        return IRQ_HANDLED;

		sta = reg_read(unicam, UNICAM_STA);      // Read the main Unicam status register
		reg_write(unicam, UNICAM_STA, sta);      // Clear the main status flags by writing back
		ista = reg_read(unicam, UNICAM_ISTA);    // Read the interrupt status register
		reg_write(unicam, UNICAM_ISTA, ista);    // Clear the interrupt flags by writing back
		fe = (ista & UNICAM_FEI || sta & UNICAM_PI0); // Set 'frame end' flag if Frame End or Packet Capture interrupt occurred
	 ...
	 * buffer forever.
	 */
	 if (fe) {                                 // If a frame-end or packet capture interrupt occurred
    bool inc_seq = unicam->frame_started; // Check if a frame was already started to decide sequence increment

    if (unicam->sync_gpio)                 // If sync GPIO is configured
        gpiod_set_value(unicam->sync_gpio, 0); // Reset sync GPIO to indicate frame end

    // Loop through all Unicam buffer nodes to handle active streams
	  /*
    ...
    for (i = 0; i < ARRAY_SIZE(unicam->node); i++) {
        struct unicam_node *node = &unicam->node[i]; // Pointer to current buffer node

        if (!node->streaming)               // Skip nodes that are not currently streaming
            continue;                       // Move to next node



if (node->cur_frm && node->cur_frm != node->next_frm) {   // Only process if current frame is valid and different from next frame
    if (!node->cur_frm->vb.vb2_buf.timestamp) {           // Drop frame if timestamp not set (FE occurred before FS)
        unicam_dbg(2, unicam, "ISR: FE without FS, dropping frame\n"); // Debug message for dropped frame
        continue;                                         // Skip to next node
    }

    unicam_process_buffer_complete(node, sequence);      // Mark current buffer as complete and pass to V4L2
    node->cur_frm = node->next_frm;                      // Swap next frame to current
    node->next_frm = NULL;                               // Clear next frame pointer
    inc_seq = true;                                      // Increment sequence number later
} else {
    node->cur_frm = node->next_frm;                      // If no swap needed, just set current frame
}

// Increment sequence number if needed to match sensor frames
if (inc_seq) {
    unicam->sequence++;                                  // Update frame sequence count
    unicam->frame_started = false;                       // Reset frame started flag
}
if (ista & UNICAM_FSI) {                               // If Frame Start interrupt occurred
    ts = ktime_get_ns();                                // Capture timestamp for frame start

    if (unicam->sync_gpio)                              // If sync GPIO configured
        gpiod_set_value(unicam->sync_gpio, 1);         // Set GPIO high at frame start

    for (i = 0; i < ARRAY_SIZE(unicam->node); i++) {   // Loop through all buffer nodes
        if (!unicam->node[i].streaming)                // Skip nodes not streaming
            continue;

        if (unicam->node[i].cur_frm)                   // If current frame exists
            unicam->node[i].cur_frm->vb.vb2_buf.timestamp = ts; // Assign timestamp
        else
            unicam_dbg(2, unicam, "ISR: [%d] Dropping frame, buffer not available at FS\n", i); // Debug if no buffer

        // Schedule dummy buffer if next frame not available or same as current
        if (!unicam->node[i].next_frm || unicam->node[i].next_frm == unicam->node[i].cur_frm) {
            unicam_schedule_dummy_buffer(&unicam->node[i]);
        } else if (unicam->node[i].cur_frm) {          // Handle repeated FS without FE
            spin_lock(&unicam->node[i].dma_queue_lock); // Lock DMA queue
            list_add_tail(&unicam->node[i].cur_frm->list, &unicam->node[i].dma_queue); // Return cur_frm to queue
            spin_unlock(&unicam->node[i].dma_queue_lock); // Unlock DMA queue

            unicam->node[i].cur_frm = unicam->node[i].next_frm; // Swap next frame to current
            unicam->node[i].next_frm = NULL;                    // Clear next frame pointer
        }
    }
}

unicam_queue_event_sof(unicam);              // Notify V4L2 of Start Of Frame event
unicam->frame_started = true;                // Mark that a new frame has started

// Handle buffer swap for FS/Line Count interrupts without frame end to avoid race conditions
if (ista & (UNICAM_FSI | UNICAM_LCI) && !fe) {
    for (i = 0; i < ARRAY_SIZE(unicam->node); i++) {  // Loop through all nodes
        if (!unicam->node[i].streaming)             // Skip nodes not streaming
            continue;

        spin_lock(&unicam->node[i].dma_queue_lock); // Lock DMA queue
        if (!list_empty(&unicam->node[i].dma_queue) &&
            !unicam->node[i].next_frm)              // If a buffer is queued and next_frm is empty
            unicam_schedule_next_buffer(&unicam->node[i]); // Schedule it as the next frame
        spin_unlock(&unicam->node[i].dma_queue_lock); // Unlock DMA queue
    }
}

return IRQ_HANDLED;                             // Return that interrupt was handled
}

// V4L2 Common IOCTL: query capabilities of the device
static int unicam_querycap(struct file *file, void *priv, struct v4l2_capability *cap) {
    struct unicam_node *node = video_drvdata(file);  // Get the node from the file pointer
    struct unicam_device *dev = node->dev;           // Get the Unicam device

    strscpy(cap->driver, UNICAM_MODULE_NAME, sizeof(cap->driver)); // Set driver name
    strscpy(cap->card, UNICAM_MODULE_NAME, sizeof(cap->card));     // Set card name

    snprintf(cap->bus_info, sizeof(cap->bus_info),  // Fill bus info string
             "platform:%s", dev_name(&dev->pdev->dev));

    cap->capabilities |= V4L2_CAP_VIDEO_CAPTURE | V4L2_CAP_META_CAPTURE; // Indicate supported capabilities
...
static int unicam_log_status(struct file *file, void *fh) {
    struct unicam_node *node = video_drvdata(file);    // Get the Unicam node from file pointer
    struct unicam_device *dev = node->dev;             // Get the parent Unicam device
    u32 reg;                                           // Temporary variable for register values

    v4l2_device_call_all(&dev->v4l2_dev, 0, core, log_status); // Query and log status of all subdevices

    unicam_info(dev, "-----Receiver status-----\n");   // Print receiver header
    unicam_info(dev, "V4L2 width/height:   %ux%u\n",  // Print configured V4L2 frame size
                node->v_fmt.fmt.pix.width, node->v_fmt.fmt.pix.height);
    unicam_info(dev, "Mediabus format:     %08x\n",   // Print media bus pixel format code
                node->fmt->code);
    unicam_info(dev, "V4L2 format:         %08x\n",   // Print V4L2 pixel format
                node->v_fmt.fmt.pix.pixelformat);

    reg = reg_read(dev, UNICAM_IPIPE);                // Read packing/unpacking register
    unicam_info(dev, "Unpacking/packing:   %u / %u\n", // Print unpacking/packing info
                get_field(reg, UNICAM_PUM_MASK),
                get_field(reg, UNICAM_PPM_MASK));

    unicam_info(dev, "----Live data----\n");         // Print live data header
    unicam_info(dev, "Programmed stride:   %4u\n",   // Print programmed stride from register
                reg_read(dev, UNICAM_IBLS));
    unicam_info(dev, "Detected resolution: %ux%u\n", // Print detected resolution from registers
                reg_read(dev, UNICAM_IHSTA),
                reg_read(dev, UNICAM_IVSTA));
    unicam_info(dev, "Write pointer:       %08x\n",   // Print current write pointer
...
/*static → function scope is limited to this source file.
int → used to indicate success (0) or failure (negative error code).
pdev is a pointer to the platform device structure provided by the kernel.
Declares a pointer to a unicam_device structure.
kzalloc():Allocates memory and initializes it to zero,Prevents garbage values.
sizeof(*unicam) → allocates memory equal to the size of the structure.
GFP_KERNEL → allocation is done in process context, safe to sleep.
kref is used to manage the lifetime of the object,Prevents premature freeing while the object is still in use.*/
static int unicam_probe(struct platform_device *pdev)
{
	...
	/*Initializes the media-controller mode based on the module parameter set at load time.*/
	unicam->mc_api = media_controller;

	/*Gets driver-specific data from Device Tree compatible match (checks if this is modern vs legacy hardware)
	Enables media-controller support if the Device Tree compatible entry indicates a non-legacy (modern) driver.*/
	if (of_device_get_match_data(&unicam->pdev->dev))
		unicam->mc_api = true;

    /*Checks if a DT property exists
	Enables media controller if explicitly requested
	Reads a boolean property from Device Tree nodes (checks if "brcm,media-controller" property exists).*/
	if (of_property_read_bool(pdev->dev.of_node, "brcm,media-controller"))
		unicam->mc_api = true;


/*without this driver is just software that can't touch the actual camera hardware.*/
/*(index 0) = Main control panel (resolution, format, capture)*/
/*this section:

maps hardware registers

acquires hardware clocks

makes the driver capable of controlling the UNICAM block*/
...
/*(index 1) = Power switch box (turns clocks on/off)*/
...
/*Obtains an optional GPIO named “sync” from Device Tree and configures it as an output initialized to low.
Retrieves the interrupt number (IRQ) assigned to the Unicam device from the platform resources.
Checks for a valid IRQ and aborts driver initialization if no interrupt resource is available.*/
...
/*Registers the Unicam interrupt handler so the driver can respond to hardware capture events.
Handles failure in IRQ registration and performs cleanup before exiting the probe function*/
...
/*Associates the media device with the platform device in the kernel*/
...
/*Initializes the serial number field (unused in this driver).*/
	strscpy(unicam->mdev.serial, "", sizeof(unicam->mdev.serial));

/*Defines the bus location string, indicating the device is a platform device.*/
	snprintf(unicam->mdev.bus_info, sizeof(unicam->mdev.bus_info),
		 "platform:%s", dev_name(&pdev->dev));

/*Sets the hardware revision information (not versioned in this case)*/
	unicam->mdev.hw_revision = 0;

/*Initializes the media device structure and prepares it for registration.*/
	media_device_init(&unicam->mdev);

/*Links the V4L2 device to the media device for media-controller support.*/
	unicam->v4l2_dev.mdev = &unicam->mdev;

/*Registers the V4L2 device with the kernel so it becomes visible to userspace*/
...
/*Registers the media controller device to expose the media pipeline*/
...
/*Initializes a control handler and reserves space for camera controls*/
...
/*Parses the Device Tree and connects camera subdevices to build the media graph*/
...
/*static: Makes this table local to this file only
const: Table cannot be modified at runtime (read-only)
struct of_device_id: Special kernel structure for Device Tree matching
unicam_of_match[]: Array name - "UNICAM OF Match Table"
*/
