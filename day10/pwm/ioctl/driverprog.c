...
static int __init ioctl_init(void){
	...
	pwm_init_state(pd, pd_state);
	duty_cyc = pwm_get_relative_duty_cycle(pd_state, 100);	
	
	ret = pwm_set_relative_duty_cycle(pd_state, 50, 100);
	if(!ret){
		pr_err("Setting duty cycle failed\n");
		return -1;
	}
