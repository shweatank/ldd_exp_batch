...
static inline u32 uart_read(struct uart_dev *ud, u32 off)
{
    return readl(ud->base + off);
}

static inline void uart_write(struct uart_dev *ud, u32 off, u32 val)
{
...
static irqreturn_t uart_irq_handler(int irq, void *dev_id)
{
    ...
    while (!(uart_read(ud, UART_FR) & UART_FR_RXFE)) {
        u32 dr = uart_read(ud, UART_DR);
    ...
    uart_write(ud, UART_ICR, UART_ICR_RXIC | UART_ICR_RTIC);
...
static void uart_hw_init(struct uart_dev *ud)
{
     ...
     */
    uart_write(ud, UART_IBRD, 26);
    uart_write(ud, UART_FBRD, 3);

    /* 8N1, FIFOs enabled */
    uart_write(ud, UART_LCRH, UART_LCRH_WLEN_8 | UART_LCRH_FEN);

    /* Enable RX + TX + UART */
    uart_write(ud, UART_CR, UART_CR_UARTEN | UART_CR_TXE | UART_CR_RXE);

    /* Enable RX + RX timeout interrupts */
    val = UART_IMSC_RXIM | UART_IMSC_RTIM;
    uart_write(ud, UART_IMSC, val);
}

static void uart_hw_deinit(struct uart_dev *ud)
{
    /* Disable interrupts and UART */
    uart_write(ud, UART_IMSC, 0);
    uart_write(ud, UART_CR, 0);
}
                          ...
                          size_t count, loff_t *ppos)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        while (uart_read(ud, UART_FR) & UART_FR_TXFF)
            cpu_relax();

        uart_write(ud, UART_DR, ch);
    }
...
static int __init uart_mod_init(void)
{
    ...
    g_uart->class = class_create(THIS_MODULE, DRIVER_NAME);
